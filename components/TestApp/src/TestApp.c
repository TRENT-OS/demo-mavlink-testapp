...
static OS_Error_t handleMavLinkConnection(void)
{
    ...
    for (;;)
    {
        ...
        if (ret != OS_SUCCESS)
        {
            Debug_LOG_ERROR("`fcAdapter_rpc_recvMessage` failed. Error code: %d",
                            ret);
            return ret;
        }
        ...
        case MAVLINK_MSG_ID_SERVO_OUTPUT_RAW:
        {
            ...
            case PILOT_STATE_ARM:
            {
                ...
                mavlink_msg_command_long_encode(
                    DUMMY_AI_SYSTEM_ID, DUMMY_AI_COMPONENT_ID, &msg_out, &cmd_out);
            ...
            {
                {
                    ...
                    mavlink_msg_set_position_target_local_ned_encode(
                        DUMMY_AI_SYSTEM_ID, DUMMY_AI_COMPONENT_ID, &msg_out, &setpoint);
                    ret = fcAdapter_rpc_sendMessage(&msg_out);
                    if (ret != OS_SUCCESS)
                    {
                ...
                }
                {
                    ...
                    mavlink_msg_manual_control_encode(
                        DUMMY_AI_SYSTEM_ID, DUMMY_AI_COMPONENT_ID, &msg_out, &ctrl);
                    ret = fcAdapter_rpc_sendMessage(&msg_out);
                    if (ret != OS_SUCCESS)
                    {
                        Debug_LOG_ERROR("`fcAdapter_rpc_sendMessage` "
...
int run()
{
    ...
    ret = fcAdapter_rpc_init(&serverAddr, FC_MAVLINK_CHANNEL);
    if (ret != OS_SUCCESS)
    {
